...
export default async function FinanceMyReportsPage() {
  ...
  return (
    <AppShell title="My Finance Reports" role={user.role}>
      <div className="space-y-6">
        <DashboardPageHeader
          eyebrow="Finance"
          title="My Finance Reports"
          description="View, edit, and track your submitted daily finance reports."
          actions={
            canCreate ? (
        ...
        {reports.length === 0 ? (
          <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-border bg-card/50 p-12 text-center">
            ...
            <h3 className="mt-4 text-lg font-semibold text-textPrimary">No Submitted Finance Reports</h3>
            <p className="mt-1 text-sm text-muted-foreground">
              {canCreate
                ? "Get started by creating your first finance report today."
                : "You have not submitted any finance reports yet."}
            </p>
            {canCreate && (
          ...
          <div className="overflow-hidden rounded-xl border border-cardBorder bg-card shadow-soft">
            {/* Table Header */}
            <div className="hidden border-b bg-muted/40 px-4 py-3 sm:grid sm:grid-cols-[1fr_1fr_1.1fr_1.1fr_1fr_auto] sm:gap-4">
            ...
            {reports.map((report) => {
              ...
              return (
                <div
                  ...
                  className="group relative flex flex-col gap-2 border-b px-4 py-3.5 transition-colors hover:bg-accent/10 sm:grid sm:grid-cols-[1fr_1fr_1.1fr_1.1fr_1fr_auto] sm:items-center sm:gap-4 last:border-b-0"
                >
                  ...
                  <div className="text-sm text-muted-foreground pointer-events-none relative z-0">
                  ...
                  <div className="flex justify-end gap-2 items-center relative z-10">
                    {report.status === "pending" && (
                      <FinanceEditButton
                        ...
                        isTMorTL={["team_member", "team_lead"].includes(user.role)}
                        isCreator={report.submittedBy === user.id}
                        userRole={user.role}
                      />
                    )}
                  </div>
