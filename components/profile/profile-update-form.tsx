...
export function ProfileUpdateForm({ profile }: { profile: ProfileData }) {
  ...
  if (activeProfile.departments?.length) {
    activeProfile.departments.forEach((d) => d.name && deptSet.add(d.name));
  }
  const departmentLabel = deptSet.size > 0 ? Array.from(deptSet).map((d) => humanizeLabel(d)).join(" · ") : "N/A";

  const rawTeamNames = new Set<string>();
  if (activeProfile.teamName) rawTeamNames.add(activeProfile.teamName);
  if (activeProfile.teamNames?.length) {
    activeProfile.teamNames.forEach((t) => t && rawTeamNames.add(t));
  }
  if (activeProfile.departments?.length) {
    activeProfile.departments.forEach((dept) => {
      if (dept.subTeams?.length) {
        dept.subTeams.forEach((st) => st && rawTeamNames.add(st));
      }
    });
  }
