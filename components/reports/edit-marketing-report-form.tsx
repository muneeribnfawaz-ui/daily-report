...
type ReportItem = {
  _id: string;
  teamName: string;
  reportType: MarketingReportValues["reportType"];
  reportDate: string;
...
export function EditMarketingReportForm({ reportId }: { reportId: string }) {
  ...
  const onSubmit = async (values: MarketingReportValues) => {
    setMessage(null);
      ...
      if (!parsed.success) {
        ...
        for (const issue of parsed.error.issues) {
          ...
          if (typeof fieldName === "string") {
            setError(fieldName as keyof MarketingReportValues, {
              type: issue.code,
  ...
  const onInvalid = (validationErrors: FieldErrors<MarketingReportValues>) => {
