    ...
    >
      <div className="flex h-full min-h-0 flex-col">
        <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-4 text-slate-200 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]">
          ...
          <Badge className="mt-4 w-fit rounded-md border border-white/10 bg-white/5 px-3 py-1 capitalize text-[11px] font-medium text-slate-100 shadow-none">
            {roleLabel}
          </Badge>
        </div>
        ...
        <div className="mt-auto space-y-3 pt-5">
          <Button
            variant="outline"
            className="w-full justify-start border-white/10 bg-white/5 text-slate-200 hover:bg-white/10 hover:text-white"
            onClick={async () => {
              await onLogout();
              onClose();
            }}
